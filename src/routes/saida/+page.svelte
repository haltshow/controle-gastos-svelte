        ...
        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
            {#each saidas as saida}
                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                    ...
                    <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap dark:text-white">{saida.origem.nome}</td>
                    <td class="py-4 px-6 text-sm font-medium text-gray-500 whitespace-nowrap dark:text-white">R$ {saida.valor.toFixed(2)}</td>
                    <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        {(saida.createdAt.getDate() < 10 ? '0' + saida.createdAt.getDate() : saida.createdAt.getDate()) + '/' + 
                        (saida.createdAt.getMonth() < 10 ? '0' + saida.createdAt.getMonth() : saida.createdAt.getMonth()) + '/' + 
                        (saida.createdAt.getFullYear())}
                    </td>
                    <td class="py-2 px-3 text-sm font-medium text-right whitespace-nowrap">
                        <a href="/saida/{saida.id}"> <button class="text-blue-600 outline outline-1 rounded-md p-1"> Editar </button> </a>
                    </td>
                    <td class="py-2 px-3 text-sm font-medium text-right whitespace-nowrap">
                        <form action="?/deleteSaida&id={saida.id}" method="POST">
